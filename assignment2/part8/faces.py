...
def get_data(S, act, grayed = True, s = 0):
    ...
    for act_type in ['actors', 'actresses']:
        ...
        for root, dirs, files in os.walk(dataset_path):
            ...
            for filename in files:
                if grayed:
                    im = imread(dataset_path + filename, mode = 'L')
                else:
                    im = imread(dataset_path + filename, mode = 'RGB')
                im = im/255.
...
def Run(S, batch_size, nEpoch, alpha = 1e-2, grayed = True, s = 0):
    ...
    TRAIN = data_utils.TensorDataset(X_TRAIN, Y_CLASSES)
    dataloader = data_utils.DataLoader(TRAIN, batch_size=batch_size, shuffle=True)
    ...
    fig.savefig(dirpath + '/part8_%i_%i_%i_%070.07f.jpg'%(S, batch_size, nEpoch, alpha))
    plt.show()
...
if __name__ == "__main__":
    ...
    #Download()
    #Resize((32,32))
    Run((32,32), 10, 2000, alpha = 1e-4, grayed = True, s = 0)
